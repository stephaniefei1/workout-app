...
function defaultState() { return { startDate: iso(monday()), selectedWeek: 1, selectedDay: 0, page: "today", planView: "week", calendarMonth: iso(monday()).slice(0, 7), unit: "lb", records: {}, installTipSeen: false }; }
...
function monthFromKey(value) { return new Date(`${value}-01T12:00:00`); }
function programSlot(date) {
  const start = new Date(`${state.startDate}T12:00:00`);
  const candidate = new Date(date); candidate.setHours(12, 0, 0, 0);
  const index = Math.round((candidate - start) / 86400000);
  return index >= 0 && index < 42 ? { week: Math.floor(index / 7) + 1, day: index % 7 } : null;
}
function syncCalendarToSelection() { state.calendarMonth = monthKey(dayDate(state.selectedWeek, state.selectedDay)); }
...
function youtubeId(url) { const match = url.match(/(?:youtu\.be\/|shorts\/|watch\?v=)([\w-]+)/); return match?.[1] || ""; }
function silentDemo(exercise) { const id = youtubeId(exercise.video); if (!id) return ""; return `<details class="silent-demo" data-silent-demo><summary><span>◌</span> View silent demo <small>keeps audio off</small></summary><div class="silent-player"><iframe title="Silent demo: ${escapeHtml(exercise.name)}" data-youtube-id="${id}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allow="encrypted-media; picture-in-picture" allowfullscreen></iframe></div></details>`; }
...
function weekCalendar() { const todayIndex = currentProgramDay(); return `<section class="card tight"><div class="days">${DAY_NAMES.map((name, day) => { const task = taskFor(state.selectedWeek, day); const d = dayDate(state.selectedWeek, day); const selected = state.selectedDay === day; const complete = isDayComplete(state.selectedWeek, day); const title = task.type === "lift" ? `Lift ${task.workout + 1}` : task.type === "cardio" ? `${task.minutes}m cardio` : "Rest"; const actual = (state.selectedWeek - 1) * 7 + day === todayIndex; return `<button class="day ${selected ? "selected" : ""} ${complete ? "complete" : ""} ${task.type === "rest" ? "rest" : ""}" data-action="day" data-day="${day}" aria-label="${name}, ${title}"><span>${name}</span><span class="day-num">${d.getDate()}</span><span class="dot" title="${actual ? "Today" : complete ? "Complete" : title}"></span><span>${title}</span></button>`; }).join("")}</div></section>`; }
function monthCalendar() {
  const viewed = monthFromKey(state.calendarMonth || monthKey(dayDate(state.selectedWeek, state.selectedDay)));
  const first = new Date(viewed.getFullYear(), viewed.getMonth(), 1, 12);
  const firstVisible = new Date(first); firstVisible.setDate(first.getDate() - ((first.getDay() + 6) % 7));
  const title = viewed.toLocaleDateString(undefined, { month: "long", year: "numeric" });
  const cells = Array.from({ length: 42 }, (_, index) => {
    const date = new Date(firstVisible); date.setDate(firstVisible.getDate() + index);
    const slot = programSlot(date); const task = slot && taskFor(slot.week, slot.day);
    const type = task?.type === "lift" ? `Lift ${task.workout + 1}` : task?.type === "cardio" ? `${task.minutes}m cardio` : task ? "Rest" : "";
    const selected = slot && slot.week === state.selectedWeek && slot.day === state.selectedDay;
    const complete = slot && isDayComplete(slot.week, slot.day);
    const outside = date.getMonth() !== viewed.getMonth();
    return `<button class="month-day ${outside ? "outside" : ""} ${slot ? "scheduled" : ""} ${selected ? "selected" : ""} ${complete ? "complete" : ""}" ${slot ? `data-action="month-day" data-week="${slot.week}" data-day="${slot.day}"` : "disabled"} aria-label="${date.toLocaleDateString(undefined, { month:"long", day:"numeric" })}${type ? `, ${type}` : ""}"><span>${date.getDate()}</span>${type ? `<small>${type}</small><i></i>` : ""}</button>`;
  }).join("");
  return `<section class="card tight"><div class="month-nav"><button class="round-btn" data-action="month-shift" data-shift="-1" aria-label="Previous month">‹</button><h3>${title}</h3><button class="round-btn" data-action="month-shift" data-shift="1" aria-label="Next month">›</button></div><div class="month-labels">${DAY_NAMES.map((day) => `<span>${day.slice(0, 1)}</span>`).join("")}</div><div class="month-grid">${cells}</div></section>`;
}
function planView() { const selectedDate = dayDate(state.selectedWeek, state.selectedDay); return `${header()}<section class="section-heading"><div><span class="eyebrow">Your schedule</span><h2>6-week plan</h2></div></section><div class="view-switch" role="group" aria-label="Calendar view"><button class="${state.planView === "week" ? "active" : ""}" data-action="plan-view" data-plan-view="week">Week</button><button class="${state.planView === "month" ? "active" : ""}" data-action="plan-view" data-plan-view="month">Month</button></div>${state.planView === "week" ? `${weekPicker()}${weekCalendar()}` : monthCalendar()}<section class="card" style="margin-top:15px"><span class="eyebrow">Week ${state.selectedWeek} · ${labelDate(selectedDate)}</span><h2>${WEEK_INFO[state.selectedWeek - 1].steps} daily steps</h2><p class="caption" style="margin-top:8px">Four full-body lifts, two low-intensity cardio sessions, and one rest day. You can select any scheduled date to log it.</p><button class="primary finish" data-action="page" data-page="today">Open selected day</button></section>${nav()}`; }
...
document.addEventListener("click", (event) => {
  ...
  if (action === "week") { state.selectedWeek = Number(el.dataset.week); syncCalendarToSelection(); render(); return; }
  if (action === "day") { state.selectedDay = Number(el.dataset.day); syncCalendarToSelection(); render(); return; }
  if (action === "plan-view") { state.planView = el.dataset.planView; if (state.planView === "month") syncCalendarToSelection(); render(); return; }
  if (action === "month-shift") { const date = monthFromKey(state.calendarMonth); date.setMonth(date.getMonth() + Number(el.dataset.shift)); state.calendarMonth = monthKey(date); render(); return; }
  if (action === "month-day") { state.selectedWeek = Number(el.dataset.week); state.selectedDay = Number(el.dataset.day); syncCalendarToSelection(); render(); return; }
...
document.addEventListener("toggle", (event) => {
  ...
  if (!details.matches?.("[data-silent-demo]") || !details.open) return;
  const frame = details.querySelector("iframe");
  if (!frame || frame.hasAttribute("src")) return;
  frame.addEventListener("load", () => frame.contentWindow?.postMessage(JSON.stringify({ event:"command", func:"mute", args:"" }), "https://www.youtube-nocookie.com"), { once:true });
  frame.src = `https://www.youtube-nocookie.com/embed/${frame.dataset.youtubeId}?playsinline=1&enablejsapi=1&rel=0&autoplay=0&mute=1`;
}, true);
